...
export const MediaZone = ({ onFilesChange }: MediaZoneProps) => {
  ...
  const deleteAction = useCallback(
    (action: Actions) => {
      setActions((prevActions) => {
        const newActions = prevActions.filter((a) => a !== action);
              ...
              <div className="relative flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-muted-foreground mt-0.5">
                    {action.from.toUpperCase()} →{" "}
                    {action.to?.toUpperCase() || "Select format"}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {action.is_error ? (
                    <Badge
                      variant="destructive"
                      className="rounded-full bg-red-50 text-red-600 border-0 px-3 py-1 font-mono text-xs uppercase tracking-tighter hover:bg-red-50 hover:text-red-600"
                    >
                    ...
                    <Badge
                      variant="default"
                      className="rounded-full bg-emerald-50 text-emerald-600 border-0 px-3 py-1 font-mono text-xs uppercase tracking-tighter hover:bg-emerald-50 hover:text-emerald-600"
                    >
                    ...
                    <Badge
                      variant="default"
                      className="rounded-full bg-amber-50 text-amber-600 border-0 px-3 py-1 font-mono text-xs uppercase tracking-tighter hover:bg-amber-50 hover:text-amber-600"
                    >
          ...
          <div className="flex flex-col items-center justify-center py-16 px-8 text-center">
            <div
              className={`
								...
								${isDragActive || isHover ? "bg-primary/10 scale-110" : "bg-primary/5"}
							`}
            >
              {isDragActive || isHover ? (
            ...
            <h3 className="text-xl font-medium text-foreground mb-2">
              {isDragActive ? "Drop your files here" : "Drag & drop your media"}
            </h3>
